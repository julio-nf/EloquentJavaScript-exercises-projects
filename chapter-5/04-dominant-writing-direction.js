...
function dominantDirection(text) {
  ...
  let maxCount = scripts.reduce((max, {count}) => Math.max(max, count), 0);
  let dominantName = scripts.filter(({count}) => count == maxCount).
                     map(({name}) => name);
  return String(SCRIPTS.filter(({name}) => name == dominantName).
